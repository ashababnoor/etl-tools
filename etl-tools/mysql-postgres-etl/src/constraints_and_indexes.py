...
for table in tables:
    ...
    if result is None:
        ...
        # Iterate through the structure of the table again to add constraints and indexes
        for column in structure:
            # Extract the column name and constraints
            column_name = column[0]
            constraints = column[2]
            #Append the constraint to the create table query
            create_table_query += " {} {}".format(column_name, constraints)
        # Create indexes
        create_table_query += ",PRIMARY KEY({})".format(column_name)
